...
Counts CloudFront cache outcomes (Hit / Miss / RefreshHit) from CloudFront
standard logs stored in S3 (tab-delimited, often .gz).

How you would talk about this script at an interview:
"I wrote a small S3-backed log analyzer that downloads recent CloudFront
standard logs, parses x-edge-result-type, and reports Hit/Miss/RefreshHit
metrics to validate caching policy."
"""
...
def print_report(counts: Dict[str, int]) -> None:
    ...
    print("\nInterpretation:")
    print("  High Hit% = lower latency and lower origin load.")
    print("  High Miss% = caching policy mismatch or uncacheable headers.")
    print("  RefreshHit = CloudFront revalidated with origin and served cached content.")
    print("=======================================================\n")

def main() -> int:
    ap = argparse.ArgumentParser(description="Count Hit/Miss/RefreshHit from CloudFront standard logs in S3.")
    ap.add_argument("--bucket", default="Class_Lab3", help="S3 bucket name")
    ap.add_argument("--prefix", default="", help="S3 prefix where logs live")
    ap.add_argument("--latest", type=int, default=3, help="Analyze latest N log objects")
    ...
    if not keys:
        ...
        print("Tip: verify with: aws s3 ls s3://Class_Lab3/ --recursive | head")
        return 2
